...
class IntentParser {
     ...
     */
    adjustConfidence(base, input, action) {
        let score = base;
     ...
     */
    extractSlots(input, action) {
        const slots = { input };
        switch (action) {
            case 'browse_web': {
                const urlMatch = input.match(/https?:\/\/[^\s]+/i);
                if (urlMatch)
                    slots['url'] = urlMatch[0];
                const domainMatch = input.match(/\bwww\.\S+/i);
                if (domainMatch)
                    slots['url'] = `https://${domainMatch[0]}`;
                break;
            }
            case 'file_op': {
            ...
            case 'recall': {
                ...
                if (recallMatch)
                    slots['query'] = recallMatch[1].trim();
                break;
            }
            case 'search': {
